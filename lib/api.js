...
export function getAllSeries() {
  const seriesDirectory = path.join(process.cwd(), '_series')
  const filenames = fs.readdirSync(seriesDirectory)

  return filenames.map(filename => {
    const file = fs.readFileSync(path.join(process.cwd(), '_series', filename), 'utf8')
...
export function getSeriesBySlug(slug) {
  const file = fs.readFileSync(path.join(process.cwd(), '_series', `${slug}.json`), 'utf8')
